...
<body>
    ...
    <script>
        ...
        function readHere(id) {
            ...
            // Create a minimal document in the new window, then build DOM nodes to avoid template parsing issues
            ...
            style.textContent = `html, body { margin: 0; padding: 0; height: 100%; background: #f8fafc; color: #0f172a; font-family: "Helvetica Neue", Helvetica, Arial, sans-serif; }
...
#viewer { height: calc(100% - 48px); }`;
  ...
  const epubUrl = ${JSON.stringify(epubUrl)};
  const jszipCdn = "https://cdnjs.cloudflare.com/ajax/libs/jszip/3.10.1/jszip.min.js";
  const epubCdn = "https://cdn.jsdelivr.net/npm/epubjs@0.3.88/dist/epub.min.js";
  ...
  function safePrev() {
    if (!navReady || !rendition) return;
    try {
      const res = rendition.prev();
  ...
  Promise.resolve()
    .then(() => (window.JSZip ? null : loadScript(jszipCdn)))
    .then(() => loadScript(epubCdn))
    .then(() => {
      setStatus("Downloading book...");
    ...
    .then((buf) => {
      ...
      const openPromise = book.open(buf, "binary");
      rendition = book.renderTo("viewer", { width: "100%", height: "100%" });
      setStatus("Rendering...");
      return Promise.resolve(openPromise).then(() => rendition.display());
    })
    .then(() => {
      enableNav(true);
    })
    .catch((err) => {
      setStatus("Failed to load EPUB: " + (err && err.message ? err.message : err));
    });
})();`;
        ...
        if (httpd_query_key_value(buf, "id", param, sizeof(param)) == ESP_OK)
        {
            ...
            if (book.id != 0)
            {
                ...
                while ((read = fread(chunk, 1, 4096, f)) > 0)
                {
                    ...
                    // Keep web activity fresh during long transfers to avoid sleep/timeouts.
                    WebServer::updateActivityTime();
                }
